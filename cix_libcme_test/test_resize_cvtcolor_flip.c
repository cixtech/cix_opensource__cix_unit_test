...
static void resize_cvtcolor_flip_usage(const char* prog)
{
    fprintf(stderr, "Usage: %s resize_cvtcolor_flip -i <input.yuv> -w <width> -h <height> -f <format> -m <mode> [-o <output>] [-W <dst_w> -H <dst_h>]\n", prog);
    fprintf(stderr, "  -i   input YUV/image file (required)\n");
    ...
    fprintf(stderr, "  -m   flip mode: none, h, v, h_v (required)\n");
    fprintf(stderr, "  -o   dump destination image to file (optional)\n");
...
int test_resize_cvtcolor_and_flip(int argc, char** argv)
{
    ...
    while ((c = getopt(argc, argv, "i:w:h:f:m:o:W:H:c:n:")) != -1) {
        ...
        default:
            resize_cvtcolor_flip_usage(argv[0] ? argv[0] : "test_cme");
            return EXIT_FAILURE;
        }
    }

    if (!input_file || src_w <= 0 || src_h <= 0 || !format_str || !mode_str) {
        fprintf(stderr, "test_resize_cvtcolor_and_flip: missing or invalid -i -w -h -f -m\n");
        resize_cvtcolor_flip_usage(argv[0] ? argv[0] : "test_cme");
        return EXIT_FAILURE;
    }
    ...
    if (src_fmt == CME_FORMAT_UNKNOWN) {
        fprintf(stderr, "test_resize_cvtcolor_and_flip: unknown format '%s'\n", format_str);
        return EXIT_FAILURE;
    }
    if (parse_flip_mode(mode_str, &flip_mode) != 0) {
        fprintf(stderr, "test_resize_cvtcolor_and_flip: unknown mode '%s'\n", mode_str);
        return EXIT_FAILURE;
    }

    cap = query_all_capability();
    if (!cap) {
        fprintf(stderr, "test_resize_cvtcolor_and_flip: query_all_capability() returned NULL\n");
        return EXIT_FAILURE;
    }

    if (!format_supported(cap->supported_output_formats, CME_FORMAT_RGB_888)) {
        fprintf(stderr, "test_resize_cvtcolor_and_flip: RGB_888 output not supported\n");
        return EXIT_FAILURE;
    }
    if (!format_supported(cap->supported_input_formats, src_fmt)) {
        fprintf(stderr, "test_resize_cvtcolor_and_flip: input format not supported\n");
        return EXIT_FAILURE;
    }
    ...
    if (error || !src) {
        fprintf(stderr, "test_resize_cvtcolor_and_flip: alloc_cme_img(src) failed (error=%d)\n", error);
        return EXIT_FAILURE;
    }

    dst = alloc_cme_img(&dst_ctrl, &error);
    if (error || !dst) {
        fprintf(stderr, "test_resize_cvtcolor_and_flip: alloc_cme_img(dst) failed (error=%d)\n", error);
        free_cme_img(src);
        return EXIT_FAILURE;
    }
    ...
    if (!fin) {
        fprintf(stderr, "test_resize_cvtcolor_and_flip: cannot open input file '%s'\n", input_file);
        free_cme_img(dst);
        free_cme_img(src);
        return EXIT_FAILURE;
    }
    ...
    for (int i = 0; i < n_round; i++) {
        ...
        ret = cme_2d_resize_cvtcolor_and_flip(src, dst, flip_mode, 1, NULL);
        unsigned long end = get_mono_time_ns();
        if (end < start) {
            fprintf(stderr, "test_resize_cvtcolor_and_flip: time error\n");
            free_cme_img(dst);
            free_cme_img(src);
            return EXIT_FAILURE;
        }
        total_time += (end - start) / 1000000.0;
        printf("test_resize_cvtcolor_and_flip: cme_2d_resize_cvtcolor_and_flip time: %lu ns (%.3f ms)\n", end - start, (end - start) / 1000000.0);
        if (ret != CME_RET_SUCCESS) {
            fprintf(stderr, "test_resize_cvtcolor_and_flip: cme_2d_resize_cvtcolor_and_flip returned %d\n", (int)ret);
            free_cme_img(dst);
            free_cme_img(src);
            return EXIT_FAILURE;
        }
    ...
    if (n_round > 0) {
        printf("test_resize_cvtcolor_and_flip: %d rounds, average time: %.3f ms\n", n_round, total_time / n_round);
    }
    printf("test_resize_cvtcolor_and_flip: OK (src %dx%d %s -> dst %dx%d RGB, mode=%s)\n",
           src_w, src_h, format_str, dst_w, dst_h, mode_str);

    if (output_file) {
        fout = fopen(output_file, "wb");
        if (!fout) {
            fprintf(stderr, "test_resize_cvtcolor_and_flip: cannot open output file '%s'\n", output_file);
            free_cme_img(dst);
            free_cme_img(src);
            return EXIT_FAILURE;
        }
        ...
        fclose(fout);
        fout = NULL;
        printf("test_resize_cvtcolor_and_flip: dumped dst to '%s' (%d bytes)\n", output_file, dst->tlength);
    }
