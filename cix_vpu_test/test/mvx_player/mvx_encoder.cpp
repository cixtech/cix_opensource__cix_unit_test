...
#define V4L2_HDR10_MASTERING_CLIP(value, field) \
    (value < V4L2_HDR10_MASTERING_##field##_LOW ? \
    V4L2_HDR10_MASTERING_##field##_LOW : \
    (value > V4L2_HDR10_MASTERING_##field##_HIGH ? \
    V4L2_HDR10_MASTERING_##field##_HIGH : value))
static char cfg_file_line_buf[CFG_FILE_LINE_SIZE];

static uint16_t convert_double_to_unsigned_16bit_q8( double x )
{
    assert( x >= 0.0 );
    assert( x < 256.0 );

    uint32_t n = (uint32_t)( x * 256.0 + 0.5 );
    assert( (n & 0xFFFF0000) == 0 );
    return (uint16_t)n;
}
...
int main(int argc, const char *argv[])
{
    ...
    mvx_argp_add_opt(&argp, '\0', "dev", true, 1, "/dev/video1", "Device.");
    ...
    mvx_argp_add_opt(&argp, 0, "rate_control", true, 1, "off", "Selects rate control type, constant/variable/standard/cvbr/off");
    ...
    mvx_argp_add_opt(&argp, 0, "gopsize", true, 1, "30", "GOP size");
    ...
    mvx_argp_add_opt(&argp, 0, "miniframe", true, 1, "0", "one input frame was seperated in several input buffer");
    ...
    mvx_argp_add_opt(&argp, 0, "svct3_level1_period", true, 1, "0", "encoder SVCT3 level 1 period: must be an even number and range from 4 to 250 (inclusive)");
    ...
    mvx_argp_add_opt(&argp, 0, "reset_ltr_period_cfg", true, 1, NULL, "Set LTR period dynamiclly, only support LDP of H264 and HEVC now");
    ...
    mvx_argp_add_opt(&argp, 0, "mulit_sps_pps", true, 1, NULL, "Support multi SPS PSS for h264 and hevc");
    ...
    mvx_argp_add_opt(&argp, 0, "lambda_scale_i", true, 1, 0, "Lambda scale for I frames, use Q16 value.");
    mvx_argp_add_opt(&argp, 0, "lambda_scale_p", true, 1, 0, "Lambda scale for P frames, use Q16 value.");
    mvx_argp_add_opt(&argp, 0, "lambda_scale_b_ref", true, 1, 0, "Lambda scale for ref-B frames, use Q16 value.");
    mvx_argp_add_opt(&argp, 0, "lambda_scale_b_nonref", true, 1, 0, "Lambda scale for nonref-B frames, use Q16 value.");

    mvx_argp_add_opt(&argp, 0, "intra_ipenalty_angular", true, 1, 0, "0~31, angular penalty for intra frame");
    mvx_argp_add_opt(&argp, 0, "intra_ipenalty_planar", true, 1, 0, "0~31, planar penalty for intra frame");
    mvx_argp_add_opt(&argp, 0, "intra_ipenalty_dc", true, 1, 0, "0~31, dc penalty for intra frame");
    mvx_argp_add_opt(&argp, 0, "inter_ipenalty_angular", true, 1, 0, "0~31, angular penalty for inter frame");
    mvx_argp_add_opt(&argp, 0, "inter_ipenalty_planar", true, 1, 0, "0~31, planar penalty for inter frame");
    mvx_argp_add_opt(&argp, 0, "inter_ipenalty_dc", true, 1, 0, "0~31, dc penalty for inter frame");
    ...
    mvx_argp_add_opt(&argp, 0, "drc_cfg", true, 1, NULL, "Dynamic rate control config file name");

#ifdef ENABLE_CPIPE
    mvx_argp_add_opt(&argp, 0, "ext_input", true, 1, "0", "use external input buffers");
#endif
    ...
    else
    {
        ...
        if (roi_file) {
            ...
            } else if (mvx_argp_is_set(&argp, "miniframe")){
                mini_cnt = mvx_argp_get_int(&argp,"miniframe",0);
                assert(mini_cnt >= 2);
    ...
    if (mvx_argp_is_set(&argp, "reset_ltr_period_cfg")) {
        const char *ltr_cfg = mvx_argp_get(&argp, "reset_ltr_period_cfg", 0);
        ...
        while (fgets(cfg_file_line_buf, CFG_FILE_LINE_SIZE, file))
        {
            ...
            {
                int reset_pic;
                int reset_ltr_period;
                if (2 == sscanf(sub_buf, "[%d-%d]", &reset_pic, &reset_ltr_period))
                {
                  if (reset_ltr_period == 0)
                        printf("--reset_ltr_period_cfg error: ltr new period must be > 0, but now get input [%s]\n", sub_buf);
                  struct v4l2_reset_ltr_period_config tmp_rest_gop;
                  tmp_rest_gop.reset_trigger_pic = reset_pic;
                  tmp_rest_gop.reset_ltr_period = reset_ltr_period;
                  input->ltr_list->push(tmp_rest_gop);
                } else {
                  printf("--reset_ltr_period_cfg file format error:[%s], should be [int-int]\n", sub_buf);
                }
                sub_buf = strtok(NULL, ",");
            }
        }
    ...
    if (strstr(mvx_argp_get(&argp, "output", 0), "/dev/null") != NULL)
        outputFile->setSkipOutput();

    if (mvx_argp_is_set(&argp, "drc_cfg")) {
        const char* drc_cfg = mvx_argp_get(&argp, "drc_cfg", 0);
        FILE *file = fopen(drc_cfg, "r");
        char *sub_buf;
        if (NULL == file) {
            cerr << "Could not read rate control config file!" <<endl;
            return 1;
        }
        while (fgets(cfg_file_line_buf, CFG_FILE_LINE_SIZE, file))
        {
            /* For each line: */
            /* buffer_index,target_birate,frame_rate */
            int32_t index = 0;
            uint32_t bitrate = 0;
            uint32_t framerate = 0;
            uint32_t minqp = 0xFFFFFFFF;
            uint32_t maxqp = 0xFFFFFFFF;
            sub_buf = strtok(cfg_file_line_buf, ",");
            if (sub_buf != NULL)
                index = atoi(sub_buf);
            sub_buf = strtok(NULL, ",");
            if (sub_buf != NULL)
                bitrate = atoi(sub_buf);
            sub_buf = strtok(NULL, ",");
            if (sub_buf != NULL)
                framerate = atoi(sub_buf);
            sub_buf = strtok(NULL, ",");
            if (sub_buf != NULL)
                minqp = atoi(sub_buf);
            sub_buf = strtok(NULL, ",");
            if (sub_buf != NULL)
                maxqp = atoi(sub_buf);
            outputFile->drc_list.push({index, bitrate, framerate, minqp, maxqp});
        }
        fclose(file);
    }
    ...
    encoder.setFrameCount(mini_cnt >= 2? mini_cnt * frames: frames);
    ...
    if (mvx_argp_is_set(&argp, "mulit_sps_pps")) {
        encoder.setMultiSPSPPS(mvx_argp_get_int(&argp, "mulit_sps_pps", 0));
    }
    ...
    if (mvx_argp_is_set(&argp, "gopsize"))
    {
        encoder.setGOPSize(mvx_argp_get_int(&argp, "gopsize", 0));
    }
    ...
    if (mvx_argp_is_set(&argp, "job_frames")) {
        encoder.setJobFrames(mvx_argp_get_int(&argp, "job_frames", 0));
    }
    ...
    if (mvx_argp_is_set(&argp, "profile"))
    {
        encoder.setProfile(mvx_argp_get_int(&argp, "profile", 0));
    }
    if (mvx_argp_is_set(&argp, "tier"))
    {
        encoder.setTier(mvx_argp_get_int(&argp, "tier", 0));
    }
    ...
    if (outputFormat == V4L2_PIX_FMT_VP9 && mvx_argp_is_set(&argp, "prob"))
    {
        encoder.setVP9ProbUpdateMode(mvx_argp_get_int(&argp, "prob", 0));
    }
    ...
    if (mvx_argp_is_set(&argp, "memory"))
    {
        ...
        if (strcmp(memory_type, "mmap") == 0) {
            ...
            encoder.setMemoryType(V4L2_MEMORY_USERPTR);
        } else {
    ...
    if (mvx_argp_is_set(&argp, "colour_description_range") || mvx_argp_is_set(&argp, "colour_primaries")
        ...
        encoder.setColorDescription((uint32_t)mvx_argp_get_int(&argp, "colour_primaries", 0),
            (uint8_t)mvx_argp_get_int(&argp, "transfer_characteristics", 0),
            (uint8_t)mvx_argp_get_int(&argp, "matrix_coeff", 0),
            (uint8_t)mvx_argp_get_int(&argp, "colour_description_range", 0));
    }
    ...
    if (mvx_argp_is_set(&argp, "sei_mastering_display") ||
        ...
        memset(&mastering, 0, sizeof(v4l2_ctrl_hdr10_mastering_display));
        struct v4l2_ctrl_hdr10_cll_info cll;
        memset(&cll, 0, sizeof(v4l2_ctrl_hdr10_cll_info));
        ...
        if (mvx_argp_is_set(&argp, "sei_mastering_display")) {
            ...
            if (10 == sscanf(mvx_argp_get(&argp, "sei_mastering_display", 0),
                ...
                mastering.display_primaries_x[0] = V4L2_HDR10_MASTERING_CLIP(x[0], PRIMARIES_X);
                mastering.display_primaries_y[0] = V4L2_HDR10_MASTERING_CLIP(y[0], PRIMARIES_Y);
                mastering.display_primaries_x[1] = V4L2_HDR10_MASTERING_CLIP(x[1], PRIMARIES_X);
                mastering.display_primaries_y[1] = V4L2_HDR10_MASTERING_CLIP(y[1], PRIMARIES_Y);
                mastering.display_primaries_x[2] = V4L2_HDR10_MASTERING_CLIP(x[2], PRIMARIES_X);
                mastering.display_primaries_y[2] = V4L2_HDR10_MASTERING_CLIP(y[2], PRIMARIES_Y);
                mastering.white_point_x = V4L2_HDR10_MASTERING_CLIP(wp_x, WHITE_POINT_X);
                mastering.white_point_y = V4L2_HDR10_MASTERING_CLIP(wp_y, WHITE_POINT_Y);
                mastering.max_display_mastering_luminance = V4L2_HDR10_MASTERING_CLIP(max_dml, MAX_LUMA);
                mastering.min_display_mastering_luminance = V4L2_HDR10_MASTERING_CLIP(min_dml, MIN_LUMA);
                has_mastering = true;
            }
        }
        if (mvx_argp_is_set(&argp, "sei_content_light")) {
            ...
            if (2 == sscanf(mvx_argp_get(&argp, "sei_content_light", 0), "SEI_CL=%d,%d", &max_cl, &min_pal)) {
                cll.max_content_light_level = max_cl < min_pal ? min_pal : max_cl;
                cll.max_pic_average_light_level = min_pal;
                has_cll = true;
            }
        }
        encoder.setHDR10Info(has_cll ? &cll : NULL, has_mastering ? &mastering : NULL);
    }
#endif
    ...
    if (mvx_argp_is_set(&argp, "gop"))
    {
        ...
        if (gop_type == 4)
        {
            if (!(mvx_argp_is_set(&argp, "svct3_level1_period") && mvx_argp_get_int(&argp, "svct3_level1_period", 0) >= 4 && mvx_argp_get_int(&argp, "svct3_level1_period", 0) <= 250))
            {
                printf("--gop_type==4 have been set, but --svct3_level1_period is wrong or don't set\n");
                exit(1);
            }
        }
        if (gop_type == 5)
    ...
    if (mvx_argp_is_set(&argp, "svct3_level1_period"))
    {
        int svct3 = mvx_argp_get_int(&argp, "svct3_level1_period", 0);
        if ( svct3 < 4 || svct3 > 250 || svct3 % 2 ==1 )
        {
            printf("--svct3_level1_period must be an even number or larger than 4\n");
            exit(1);
        }
        if( !(mvx_argp_is_set(&argp, "gop") && mvx_argp_get_int(&argp, "gop", 0) == 4) )
        {
            printf("--svct3_level1_period have been set, but gop_type != 4\n");
            exit(1);
        }
        ...
        && mvx_argp_is_set(&argp, "src_crop_width") && mvx_argp_is_set(&argp, "src_crop_height"))
    {
        ...
        src_crop.left = mvx_argp_get_int(&argp, "src_crop_x", 0);
        src_crop.top = mvx_argp_get_int(&argp, "src_crop_y", 0);
        src_crop.width = mvx_argp_get_int(&argp, "src_crop_width", 0);
        src_crop.height = mvx_argp_get_int(&argp, "src_crop_height", 0);
        assert(src_crop.left >=0 && (uint32_t)src_crop.left < width);
        assert(src_crop.top >=0 && (uint32_t)src_crop.top < height);
        assert(src_crop.width >0 && src_crop.width <= width);
        assert((src_crop.width +src_crop.left)  >0 && (src_crop.width+src_crop.left) <= width);
        assert(src_crop.height >0 && src_crop.height <= height);
        assert((src_crop.height +src_crop.top)  >0 && (src_crop.height +src_crop.top) <= height);
        ...
        {
            assert(mvx_argp_get_int(&argp, "crop_left", 0) < (int)src_crop.width );
            assert(mvx_argp_get_int(&argp, "crop_top", 0) < (int)src_crop.height );
            assert(mvx_argp_get_int(&argp, "crop_right", 0) <= (int)src_crop.width );
            assert(mvx_argp_get_int(&argp, "crop_bottom", 0) <= (int)src_crop.height );
            assert(mvx_argp_get_int(&argp, "crop_right", 0) - mvx_argp_get_int(&argp, "crop_left", 0)  <= (int)src_crop.width );
            assert(mvx_argp_get_int(&argp, "crop_bottom", 0) - mvx_argp_get_int(&argp, "crop_top", 0)  <= (int)src_crop.height );
        }
        ...
        && mvx_argp_is_set(&argp, "crop_top") && mvx_argp_is_set(&argp, "crop_bottom"))
    {
        ...
        crop.left = mvx_argp_get_int(&argp, "crop_left", 0);
        crop.top = mvx_argp_get_int(&argp, "crop_top", 0);
        crop.width = width - crop.left - mvx_argp_get_int(&argp, "crop_right", 0);
        crop.height = height - crop.top - mvx_argp_get_int(&argp, "crop_bottom", 0);
        encoder.setEncCrop(&crop);
    }

    InputFile *input = dynamic_cast<InputFile*>(inputFile);
#ifdef ENABLE_CPIPE
    if (mvx_argp_is_set(&argp, "ext_input"))
    {
        encoder.setExtInput(mvx_argp_get(&argp, "ext_input", 0));
        encoder.setMemoryType(V4L2_MEMORY_DMABUF);
    }
#endif

    input->setRewind(mvx_argp_get_int(&argp, "rewind", 0));
    if (mvx_argp_is_set(&argp, "rewind_frames"))
    {
        size_t input_frame_count_loop = mvx_argp_get_int(&argp, "rewind_frames", 0);
        if (input_frame_count_loop > SUPPORT_BUF_CNT_MAX)
        {
            printf("rewind frames for each loop must be no more than 32\n");
            exit(1);
        }
        input->setRewind(true);
        size_t file_frame_num = input->getFileFrameNum();
        size_t cached_frame_num = file_frame_num > 1 ? std::min(file_frame_num, input_frame_count_loop)
                                                     : input_frame_count_loop;
        input->setCachedFrame(cached_frame_num);
        encoder.setFrameBufCnt(cached_frame_num);
    } else {
        encoder.setFrameBufCnt(mvx_argp_get_int(&argp, "framebuffer_cnt", 0));
    }

    if (mvx_argp_is_set(&argp, "priority"))
    {
        int priority = mvx_argp_get_int(&argp, "priority", 0);
        if (priority < 0 || priority > 3) {
            priority = V4L2_SESSION_PRIORITY_NORMAL; // default to normal
        }
        encoder.setPriority(priority);
    }

    if (mvx_argp_is_set(&argp, "lambda_scale_i") || mvx_argp_is_set(&argp, "lambda_scale_p")
        || mvx_argp_is_set(&argp, "lambda_scale_b_ref") || mvx_argp_is_set(&argp, "lambda_scale_b_nonref"))
    {
        struct v4l2_mvx_lambda_scale lambda_scale;
        double x;
        memset(&lambda_scale, 0, sizeof(struct v4l2_mvx_lambda_scale));
        if(mvx_argp_is_set(&argp, "lambda_scale_i"))
        {
            x = ((double)mvx_argp_get_int(&argp, "lambda_scale_i", 0)) / 1000;
            lambda_scale.lambda_scale_i_q8             = convert_double_to_unsigned_16bit_q8(x);
            lambda_scale.lambda_scale_sqrt_i_q8        = convert_double_to_unsigned_16bit_q8(sqrt(x));
        }
        if(mvx_argp_is_set(&argp, "lambda_scale_p"))
        {
            x = ((double)mvx_argp_get_int(&argp, "lambda_scale_p", 0)) / 1000;
            lambda_scale.lambda_scale_p_q8             = convert_double_to_unsigned_16bit_q8(x);
            lambda_scale.lambda_scale_sqrt_p_q8        = convert_double_to_unsigned_16bit_q8(sqrt(x));
        }
        if(mvx_argp_is_set(&argp, "lambda_scale_b_ref"))
        {
            x = ((double)mvx_argp_get_int(&argp, "lambda_scale_b_ref", 0)) / 1000;
            lambda_scale.lambda_scale_b_ref_q8             = convert_double_to_unsigned_16bit_q8(x);
            lambda_scale.lambda_scale_sqrt_b_ref_q8        = convert_double_to_unsigned_16bit_q8(sqrt(x));
        }
        if(mvx_argp_is_set(&argp, "lambda_scale_b_nonref"))
        {
            x = ((double)mvx_argp_get_int(&argp, "lambda_scale_nonref", 0)) / 1000;
            lambda_scale.lambda_scale_b_nonref_q8             = convert_double_to_unsigned_16bit_q8(x);
            lambda_scale.lambda_scale_sqrt_b_nonref_q8        = convert_double_to_unsigned_16bit_q8(sqrt(x));
        }
        encoder.setEncLambdaScale(&lambda_scale);
    }

    if (mvx_argp_is_set(&argp, "intra_ipenalty_angular"))
    {
        encoder.setEncIntraIpenaltyAngular(mvx_argp_get_int(&argp, "intra_ipenalty_angular", 0));
    }
    if (mvx_argp_is_set(&argp, "intra_ipenalty_planar"))
    {
        encoder.setEncIntraIpenaltyPlanar(mvx_argp_get_int(&argp, "intra_ipenalty_planar", 0));
    }
    if (mvx_argp_is_set(&argp, "intra_ipenalty_dc"))
    {
        encoder.setEncIntraIpenaltyDc(mvx_argp_get_int(&argp, "intra_ipenalty_dc", 0));
    }
    if (mvx_argp_is_set(&argp, "inter_ipenalty_angular"))
    {
        encoder.setEncInterIpenaltyAngular(mvx_argp_get_int(&argp, "inter_ipenalty_angular", 0));
    }
    if (mvx_argp_is_set(&argp, "inter_ipenalty_planar"))
    {
        encoder.setEncInterIpenaltyPlanar(mvx_argp_get_int(&argp, "inter_ipenalty_planar", 0));
    }
    if (mvx_argp_is_set(&argp, "inter_ipenalty_dc"))
    {
        encoder.setEncInterIpenaltyDc(mvx_argp_get_int(&argp, "inter_ipenalty_dc", 0));
    }
